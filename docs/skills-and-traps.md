...
| `author` | auto | the free-form `agent` **label** the caller passed — "which job was this", not identity |
| `author_user` | auto | the **real** author: the user the token names, from the request, not settable from any argument. `skill_search(author=…)` filters on this one |
...
Two surfaces, and there is **no `trap_catalog` and no REST route** — unlike skills and tools, a
trap is reached by search or through the node it is attached to:

```sh
trap_search("regex parse")           # FTS over title, what_failed, symptom, root_cause,
                                     # instead and subject_key
trap_search()                        # no query: the most recently recorded traps
trap_get("<trap_id>")                # one trap in full
```
`trap_search` also takes `include_retired`, `limit` (capped at 100), `author=` and
`response_format`. **`node_id=` is a post-filter, not a query**: it is applied to the candidate page
the search already produced, so on a busy project it can miss an older trap on that node. The
exhaustive route for one node is `graph_get`, which selects by `node_id` in SQL.

**Traps also arrive unrequested.** `graph_get` on a node returns its traps, and `graph_search`
attaches `related_traps` + a `trap_warning` when a query matches one — see *Making sure they are
actually seen* below. That is the route that works when nobody thought to search.
...
| `verified_how` | | free text, by convention one of `measured` \| `reproduced` \| `inferred` — **not validated**, so anything is stored verbatim |
| `confidence` | | `low` \| `medium` \| `high` (default `medium`) — this one **is** validated; anything else is refused |
| `status` | auto | `active` \| `disputed` \| `retired`, set via `trap_status` (also validated) |
| `author` / `author_user` | auto | the `agent` label, and the user the token names. `trap_search(author=…)` filters on the latter |
...
> `hivemind-admin --project <p> embed` to re-embed.

(That is the whole message; the text ends there.) The other shape is *no* embeddings at all:
`semantic search is INACTIVE: no embeddings stored for <kind>s. Run … embed to enable it.`

Vectors are L2-normalised float32 in SQLite and cosine is a dot product; at a few thousand items
brute force costs microseconds and needs no vector index (sqlite-vec is still pre-1.0). There is
**one row per item**, keyed `(kind, item_id)`, so re-embedding under another backend replaces the
vector rather than storing a second one — which is exactly why a backend switch reads as "0 vectors
for the active backend". Embeddings are written on publish, **after** the write transaction commits
— the database write lock is not reentrant, so embedding inside it would deadlock. Backfill existing
