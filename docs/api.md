...
`schema_apply`. Artifacts: `artifact_ref`, `artifact_attach`, `artifact_refs`. Tools:
...
## Clients
- `hivemind` CLI (`node/edge/search/neighbors/schema/artifact/tool/guide`), config from
  `HIVEMIND_SERVER_URL` + `HIVEMIND_TOKEN`.
