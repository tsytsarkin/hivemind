...
def main(argv=None):
    ...
    ap.add_argument("--key-env", help="environment variable containing the listen key")
    ap.add_argument("--instance-id", help="non-secret marker for safe session cleanup")
    ...
    if key or args.ticket:
        sep = "&" if "?" in url else "?"
        url += sep + ("key=%s" % key if key else "ticket=%s" % args.ticket)
